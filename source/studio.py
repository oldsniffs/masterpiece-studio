...
class Studio(QMainWindow):
    ...
    def collect_song_parameters(self):
        song_parameters = {'measures': self.configuration['measures'], 'timesig': self.configuration['timesig'],
                           'key': self.assemble_key(), 'key_scale': self.configuration['keysig_scale'],
                           'lh_ranges': [self.lh_range1_lynotation, self.lh_range2_lynotation],
                           'rh_ranges': [self.rh_range1_lynotation, self.rh_range2_lynotation],
                           'pdf': self.configuration['pdf'], 'midi': self.configuration['midi']}

        return song_parameters
    ...
    def load_configuration(self):
        ...
        QFileDialog.getOpenFileName(self.ui.dialog_anchor, "Load Configuration", f"{os.getcwd()}/configurations",
                                    "*.cf", options=QFileDialog.DontUseNativeDialog)[0]:
    ...
    def select_song_folder(self):
        if directory := QFileDialog.getExistingDirectory(self.ui.dialog_anchor, "Select Folder",
                                                         os.getcwd() + "/output",
    ...
    # Notation formatting and output
    def lywrite_notation(self, notation):
        rh_ly = self._lywrite_hand(notation.left_notation)
        lh_ly = self._lywrite_hand(notation.right_notation)
